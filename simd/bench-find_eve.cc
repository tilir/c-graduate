...
int find_simd(const int *a, int n, int x) {
  auto sub = eve::algo::as_range(a, a + n);  
  auto pos = eve::algo::find(sub, x);
  if (pos == sub.end())
    return -1;
  return pos - sub.begin();
}

void measure_find(find_ty f, const int *a, int n, int reps, int q) {
  double macc;
  int i, j;
